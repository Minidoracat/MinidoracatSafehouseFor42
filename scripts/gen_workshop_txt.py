...
- EN 描述每行加 description= 前綴（空行也輸出 description=），工作區行尾 CRLF、UTF-8 無 BOM
- 翻譯包（單語）把 SRC 改成 STEAM_DESCRIPTION.md

首發流程：先跑本腳本預建（無 id=）→ 遊戲上傳並回寫 id= → 再跑本腳本補描述 → commit。
...
DST = os.path.join(REPO, "MOD", "MinidoracatSafehouseFor42", "workshop.txt")

TITLE = "Minidoracat Safehouse for B42"
